...
async function addVolunteer (user_id, skills, availability){
    ...
    try{
        ...
        const [result] = await pool.query('INSERT INTO volunteers (user_id, skills, availability) VALUES (?, ?, ?)',
...
try {
const [result] = await pool.query('SELECT role FROM users WHERE id = ?', [id]);
...
try {
const [rows] = await pool.query("select * from volunteers where volunteer_id = ?" , [id]);
return rows;
    } catch (err) {
throw err;
    }
...
module.exports = {addVolunteer , isVolunteer , getAllVolunteers , getVolunteer , updateVolunteer , deleteVolunteer};