...
const {addVolunteer , isVolunteer , getAllVolunteers , getVolunteer , updateVolunteer , deleteVolunteer} = require ('../database_managment/volunteers_db')
...
if (decoded.role !== 'volunteer' && decoded.role !== 'admin') {
   res.status(403).json({ message: 'Only volunteers or admins can create volunteer profiles.' });
}
...
if (decoded.role === 'volunteer' && decoded.id !== user_id) {
   res.status(403).json({ message: 'Volunteers can only create their own profile.' });
}

const result = await addVolunteer(user_id, skills, availability);

res.status(201).json({message: 'Volunteer profile created successfully.',data: result });
} catch (err) {
...
if (decoded.role.toLowerCase() !== "admin" && decoded.id != volunteerId) {
  return res.status(403).json({ message: "You are not authorized", data: "" });
}
...
res.status(200).json({ message: "Volunteer updated successfully", data: result });
} catch (err) {
console.error(err);
res.status(500).json({ message: "Internal server error",error : err.message });
}
...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (decoded.role.toLowerCase() !== "admin") {
      return res.status(403).json({ message: "Only admin can delete volunteers.", data: "" });
    }
    ...
    res.status(200).json({ message: "Volunteer deleted successfully", data: result });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Internal server error", error : err.message });
  }
